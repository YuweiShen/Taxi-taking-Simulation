...
class Drv():
    ...
    def move(self, time_passed):
        if self.state == 0:
            ...
            olist = [x for x in global_var.list_order if x.drvkey == self.phone]
            order = olist[0]
            goto_aim(self, order, 'order', current_time)
        elif self.state == 3:
            if current_time - self.wait_time >= 5:
                self.state = 2
                self.wait_time = None
        elif self.state == 4:
            if current_time - self.caught_time >= 10:
                self.state = 2
                self.caught_time = None
        self.log = self.pos.left
        self.lat = self.pos.top
...
if __name__ == '__main__':
    ...
    _thread.start_new_thread(main_houtai, ())
    game_intro()
